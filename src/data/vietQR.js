class VietQR {
  constructor(template = "Chỉ hiển thị QR") {
    ...
    this.crc = "";
    this.template = template; // Store selected template
  }
  ...
  build() {
    const templateData =
      this.template !== "Chỉ hiển thị QR"
        ? `65${this.convertLength(this.template.length)}${this.template}`
        : "";
